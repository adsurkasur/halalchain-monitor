...
export function RealTrackingMap({ className, compact = false, mode = "all" }: { className?: string; compact?: boolean; mode?: "all" | "single" }) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border border-border z-0 h-full min-h-[300px] w-full ${className}`}>
      <MapContainer center={center} zoom={compact ? 7 : 8} style={{ height: '100%', width: '100%' }}>
        <TileLayer
        ...
        {mode === "all" ? (
          <>
            ...
            <Polyline positions={routePantura} color="var(--primary)" weight={4} opacity={0.6} dashArray="8, 8" />
            <Polyline positions={routeBandung} color="var(--primary)" weight={4} opacity={0.7} />
            
            {customIcon && activeVehicles.map((v, i) => (
