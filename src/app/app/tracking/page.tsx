...
export default function Tracking() {
  return (
    <DashboardLayout
      title="Live tracking"
      subtitle="Real-time fleet, route, and shipment telemetry."
    >
        ...
        <div className="rounded-xl border border-border bg-surface shadow-card flex flex-col overflow-hidden">
          <div className="border-b border-border p-3">
            <div className="relative">
              ...
              <Input className="h-9 pl-9 border-border bg-surface" placeholder="Search shipments" />
            </div>
            ...
            {shipments.map((s, i) => (
              <li key={s.id}>
                <Link
                  ...
                  className={`block px-3 py-3 transition-colors hover:bg-surface-muted ${i === 0 ? "bg-accent" : ""}`}
                >
        ...
        <div className="rounded-xl border border-border bg-surface p-4 shadow-card overflow-y-auto">
            ...
            {[
              ...
              >
                ...
                <span className="flex items-center gap-2 text-xs text-muted-foreground">
                  {x.v}
                  <StatusBadge tone={x.t}>OK</StatusBadge>
                </span>
              </li>
            ))}
          </ul>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-status-safe" /> On schedule
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-status-warning" /> Warning
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-status-critical" /> Critical
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-status-active" /> Active tracking
            </span>
          </div>
        </div>
      </div>
