    ...
    >
          ...
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Real-time operational state across the fleet.
              </p>
            </div>
            <Link
              href="/app/shipments"
                ...
                {shipments.slice(0, 6).map((s) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-xs text-muted-foreground">
                      <div className="truncate max-w-[180px]">{s.origin}</div>
                      <div className="truncate max-w-[180px]">→ {s.destination}</div>
                    </td>
              ...
              {[
                ...
                <div key={r.l}>
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                    <div
                      className={`h-full ${r.tone === "safe" ? "bg-status-safe" : r.tone === "warning" ? "bg-status-warning" : "bg-status-critical"}`}
                      style={{ width: r.w }}
                    />
                  </div>
                </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-surface shadow-card">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            ...
            <Link href="/app/alerts" className="text-xs font-medium text-primary hover:underline">
              All alerts →
            </Link>
          </div>
          <ul className="divide-y divide-border">
            {alerts.slice(0, 5).map((a) => (
              <li key={a.id} className="flex items-start gap-3 px-5 py-3">
                <StatusBadge tone={statusTone(a.severity)} dot={false} className="mt-0.5">
                  {a.severity}
                </StatusBadge>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-muted-foreground">
                    {a.message} · <span className="font-mono text-primary">{a.shipment}</span>
                  </div>
                </div>
              </li>
            ))}
        ...
        <div className="rounded-xl border border-border bg-surface shadow-card">
          <div className="border-b border-border px-5 py-4">
            <h2 className="text-sm font-semibold">Operational timeline · SHP-24819</h2>
          </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {[
          {
            i: Thermometer,
            l: "Avg fleet temperature",
            v: "-17.8°C",
            d: "Within profile · -22 to -16°C",
            tone: "safe" as const,
          },
          {
            i: Droplets,
            l: "Avg humidity",
            v: "82%",
            d: "Stable across reefers",
            tone: "safe" as const,
          },
          {
            i: ShieldCheck,
            l: "Halal seal integrity",
            v: "98.4%",
            d: "3 seals require review",
