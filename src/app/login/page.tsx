    ...
    <div className="grid min-h-screen md:grid-cols-2">
      ...
      <div className="relative hidden flex-col justify-between overflow-hidden bg-sidebar p-10 text-sidebar-foreground md:flex">
        ...
        <div className="relative z-10">
          ...
          <p className="mt-3 max-w-md text-sm text-sidebar-foreground/70">Every shipment. Every sensor. Every signature. One auditable platform — from dock to receiver.</p>
          <div className="mt-10 grid max-w-md grid-cols-3 gap-4 border-t border-sidebar-border pt-6">
            {[{ v: "99.97%", l: "Uptime" }, { v: "12k+", l: "Shipments" }, { v: "<3 min", l: "Response" }].map((s) => (
              <div key={s.l}><div className="text-xl font-semibold">{s.v}</div><div className="text-[10px] uppercase tracking-wider text-sidebar-foreground/60">{s.l}</div></div>
            ))}
          </div>
        </div>
        <div className="relative z-10 flex items-center gap-2 text-xs text-sidebar-foreground/60"><Anchor className="h-3 w-3" /> STELINA-integrated · MUI LPPOM compliant</div>
      </div>
      ...
      <div className="flex items-center justify-center bg-background p-6 md:p-10">
        <div className="w-full max-w-md">
          ...
          <div className="my-6 flex items-center gap-3 text-xs text-muted-foreground">
            <div className="h-px flex-1 bg-border" /> Single sign-on <div className="h-px flex-1 bg-border" />
          </div>
